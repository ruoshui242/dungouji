...
if grep -qs "CentOS release 6" /etc/redhat-release; then
	...
	if [[ "$down_files" =~ ^[yY]$ ]]; then
		echo "正在下载gost2.11版本"
		wget https://github.com/ruoshui242/dungouji/releases/download/final/gost -O /usr/bin/gost
		chmod +x /usr/bin/gost
		echo "正在下载被控端"
		wget https://github.com/ruoshui242/dungouji/releases/download/final/ip_table -O /usr/bin/ip_table
		chmod +x /usr/bin/ip_table
		echo "正在下载brook"
		wget https://github.com/ruoshui242/dungouji/releases/download/final/brook -O /usr/bin/brook
...
beikong1_chushihua(){
	...
	if [[ "$down_files_1" =~ ^[yY]$ ]]; then
		...
		chmod +x /usr/bin/gost
		echo "正在下载被控端"
		wget https://github.com/ruoshui242/dungouji/releases/download/final/iptables_gost -O /usr/bin/iptables_gost
